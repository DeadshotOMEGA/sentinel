...
import { BadgeStatusRepository } from '../../../src/repositories/badge-status-repository'
import type { CreateBadgeStatusInput, UpdateBadgeStatusInput } from '@sentinel/types'

describe('BadgeStatusRepository Integration Tests', () => {
  const testDb = new TestDatabase()
  let repo: BadgeStatusRepository

  beforeAll(async () => {
    await testDb.start()
    repo = new BadgeStatusRepository(testDb.prisma!)
  }, 60000)
  ...
  describe('create', () => {
    it('should create a badge status with all fields', async () => {
      const input: CreateBadgeStatusInput = {
        code: 'ACTIVE',
        name: 'Active Badge',
        description: 'Badge is currently active',
        color: '#00FF00',
      }
      ...
      expect(status.name).toBe('Active Badge')
      expect(status.description).toBe('Badge is currently active')
      expect(status.color).toBe('#00FF00')
      expect(status.createdAt).toBeInstanceOf(Date)
      expect(status.updatedAt).toBeInstanceOf(Date)
    })

    it('should create badge status without optional fields', async () => {
      const input: CreateBadgeStatusInput = {
        code: 'LOST',
        name: 'Lost',
      }
      ...
      expect(status.code).toBe('LOST')
      expect(status.name).toBe('Lost')
  ...
  describe('findById', () => {
    it('should find existing badge status by ID', async () => {
  ...
  describe('findByCode', () => {
    it('should find existing badge status by code', async () => {
      await repo.create({
        code: 'UNIQUE',
        name: 'Unique Status',
      })
  ...
  describe('update', () => {
    ...
    it('should update multiple fields', async () => {
      ...
      const update: UpdateBadgeStatusInput = {
        code: 'NEW',
        name: 'New Status',
    ...
    it('should throw error when updating non-existent status', async () => {
      ...
      ).rejects.toThrow('Badge status not found')
    })
  ...
  describe('delete', () => {
    it('should delete existing badge status', async () => {
      const created = await repo.create({
        code: 'DELETE_ME',
        name: 'To Delete',
      })
    ...
    it('should prevent deletion when status is in use', async () => {
      ...
      // Create a badge using this status
      await testDb.prisma!.badge.create({
        data: {
          serialNumber: 'B001',
          status: 'ACTIVE',
          assignmentType: 'none',
          badgeStatusId: status.id,
        },
      })

      await expect(repo.delete(status.id)).rejects.toThrow(
        'Cannot delete badge status with 1 assigned badges'
      )
    })
  })

  describe('getUsageCount', () => {
    it('should return 0 for status with no badges', async () => {
      const status = await repo.create({
        code: 'UNUSED',
        name: 'Unused Status',
      })
    ...
    it('should return correct count for status with badges', async () => {
      const status = await repo.create({
        code: 'POPULAR',
        name: 'Popular Status',
      })

      await testDb.prisma!.badge.createMany({
        data: [
          {
            serialNumber: 'B001',
            status: 'ACTIVE',
