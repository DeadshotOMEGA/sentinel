...
export function VisitorSelfSigninModal({
  ...
  const memberSearchQuery = useQuery({
    ...
    enabled: open && hostSearchEnabled && deferredHostSearch.length >= 2,
    queryFn: async () => {
  ...
  const registerFieldRef = (name: KeyboardFieldName) => (element: KeyboardFieldElement | null) => {
    fieldRefs.current[name] = element
  }

  const activateKeyboardField = (name: KeyboardFieldName, mode: TouchKeyboardMode = 'keyboard') => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} dismissible={!createVisitor.isPending}>
      <DialogContent
        size="full"
        showCloseButton={!createVisitor.isPending && completionState === null}
        className="flex max-w-6xl flex-col overflow-hidden"
        style={{ maxHeight: 'calc(100vh - var(--space-6))' }}
      >
        <div className="flex min-h-0 flex-1 flex-col">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">Visitor Self Sign-In</DialogTitle>
            <DialogDescription>
          ...
          <div style={{ marginBottom: 'var(--space-3)' }}>
            <ul className="steps steps-vertical lg:steps-horizontal w-full">
              {steps.map((item) => (
                <li
                  key={item.label}
                  className={`step ${item.active ? 'step-primary' : ''} text-sm`}
                >
                  {item.label}
                </li>
              ))}
          ...
          {completionState ? (
            <div
              className="flex flex-col items-center text-center bg-success-fadded text-success-fadded-content rounded"
              style={{ gap: 'var(--space-4)', padding: 'var(--space-8) var(--space-6)' }}
            >
              ...
              <div
                className="rounded bg-base-100 text-base-content"
              ...
              <button
                type="button"
                className="btn btn-success btn-lg min-w-56"
            ...
            <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
              <input
                type="hidden"
                {...register('visitType', { required: 'Select a visitor type' })}
              />
              <input
                type="hidden"
              ...
              >
                {step === 1 && (
                  <div className="flex flex-col" style={{ gap: 'var(--space-4)' }}>
                    <div
                      className="bg-info-fadded text-info-fadded-content rounded"
                      style={{ padding: 'var(--space-3) var(--space-4)' }}
                    >
                  ...
                  >
                    ...
                    {visitType === 'recruitment' && (
                      <fieldset className="fieldset lg:col-span-2">
                        ...
                        <select
                          className={`select select-md w-full ${errors.recruitmentStep ? 'select-error' : ''}`}
                  ...
                  <div className="flex flex-col" style={{ gap: 'var(--space-4)' }}>
                    {visitType === 'contractor' ? (
                      <div
                        ...
                        style={{ padding: 'var(--space-3) var(--space-4)' }}
                      >
                        Tell us what work you are here to complete today so Unit staff can direct
                        you appropriately.
                      </div>
                    ) : (
                      <>
                    ...
                    {hostSearchEnabled && (
                      <div className="rounded bg-base-200" style={{ padding: 'var(--space-4)' }}>
                        <div className="flex flex-col" style={{ gap: 'var(--space-3)' }}>
                          <div>
                            <p className="font-semibold">Search for the member you are visiting</p>
                            <p className="text-sm text-base-content/70">
                              If you cannot find them, enter fallback details below so staff can
                              assist you.
                            </p>
                          </div>
                          ...
                          {selectedHost ? (
                            <div
                              className="flex items-center justify-between rounded bg-base-100"
                ...
                {step === 4 && (
                  <div className="flex flex-col" style={{ gap: 'var(--space-4)' }}>
                    <div
                      className="grid grid-cols-1 lg:grid-cols-2 bg-base-200 rounded"
                    ...
                    >
                      ...
                      <div className="flex flex-col" style={{ gap: 'var(--space-2)' }}>
                        ...
                        <p>
                          {[trimValue(rankPrefix), firstName, lastName].filter(Boolean).join(' ')}
                        </p>
                    ...
                    <div
                      className="bg-success-fadded text-success-fadded-content rounded"
                      style={{ padding: 'var(--space-3) var(--space-4)' }}
                    >
                      <div className="flex items-start" style={{ gap: 'var(--space-2)' }}>
                        <ShieldCheck className="h-5 w-5 mt-0.5 shrink-0" />
                        <p>
                          Review your information carefully. When you finish, the kiosk will show
                          where you need to report next.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {!keyboardVisible && (
                <DialogFooter
                  className="shrink-0 flex-wrap items-center justify-between"
                  style={{ marginTop: 'var(--space-4)', rowGap: 'var(--space-2)' }}
                >
                  <div className="flex" style={{ gap: 'var(--space-2)' }}>
                    ...
                    <button
                      type="button"
                      className="btn btn-ghost btn-md lg:btn-lg"
                      onClick={() => onOpenChange(false)}
                    >
                      Cancel
                    </button>
              ...
              {(step === 2 || step === 3) && (
                <div className="shrink-0" style={{ marginTop: 'var(--space-4)' }}>
                  {activeKeyboardField ? (
                    ...
                    <div
                      className="rounded-b bg-base-200 text-base-content/70"
                      style={{ padding: 'var(--space-4)' }}
                    >
