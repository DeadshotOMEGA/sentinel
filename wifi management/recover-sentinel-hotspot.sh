...
CONNECTION_NAME="${1:-Sentinel Hotspot}"
...
LOCK_FILE="${LOCK_FILE:-/run/lock/sentinel-hotspot-recover.lock}"

log() {
  printf '[hotspot-recover] %s\n' "$*"
}
...
exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  die "Hotspot recovery is already running"
fi
...
log "Reapplying the known-good hotspot radio settings"
...
if [[ "${KEY_MGMT}" == "sae" ]]; then
  log "WPA3-only (sae) has been unreliable on this adapter; if recovery fails, switch the profile back to wpa-psk"
fi
...
if ! reset_driver_binding "${DRIVER_UNBIND}" "${DRIVER_BIND}" "${USB_FUNCTION_NAME}"; then
  ...
  reset_usb_device "${USB_AUTHORIZED}" "${USB_UNBIND}" "${USB_BIND}" "${USB_DEVICE_NAME}" || die "Could not reset ${USB_DEVICE_NAME}"
fi
