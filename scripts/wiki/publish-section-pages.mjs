...
const WIKI_ROOT = path.join(ROOT, 'docs/wiki')
const ASSETS_ROOT = path.join(ROOT, 'docs/wiki/assets/wiki-dashboard/operations')
const SECTION_ROOTS = [
  'docs/wiki/operations/history',
  'docs/wiki/admin/members',
  'docs/wiki/operations/events',
  'docs/wiki/operations/schedules',
...
function slugFromSource(source) {
  return path.relative(WIKI_ROOT, source).replace(/\.md$/, '').split(path.sep).join('/')
}

function descriptionForSlug(slug) {
  if (slug.startsWith('admin/members/')) return 'Sentinel member record help page.'
  if (slug.startsWith('operations/history/')) return 'Sentinel Check-In/Out History help page.'
  if (slug.startsWith('operations/events/')) return 'Sentinel event planning help page.'
  if (slug.startsWith('operations/schedules/')) return 'Sentinel schedule planning help page.'
  return 'Sentinel operational help page.'
}
...
async function publishPage({ baseUrl, apiKey, locale, source, existingPages, dryRun }) {
  ...
  const tags = ['operations', 'help', slug.split('/')[1] ?? 'sentinel']
...
async function main() {
  ...
  if (!args.skipAssets) {
    const assetFiles = (await walkFiles(ASSETS_ROOT)).filter((file) => file.endsWith('.png')).sort()
  ...
  const pageFiles = (await Promise.all(SECTION_ROOTS.map((root) => walkFiles(root))))
    .flat()
  ...
  console.log(`${args.dryRun ? 'Dry-ran' : 'Published'} ${pageFiles.length} section pages.`)
}
