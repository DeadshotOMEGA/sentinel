...
export class ListItemRepository {
  ...
  async findByType(listType: ListType): Promise<ListItem[]> {
    const rows = await prisma.$queryRaw<ListItemRow[]>`
  ...
  async create(listType: ListType, data: CreateListItemInput): Promise<ListItem> {
    ...
    if (displayOrder === undefined) {
      const maxOrderResult = await prisma.$queryRaw<Array<{ max_order: number | null }>>`
  ...
  async update(id: string, data: UpdateListItemInput): Promise<ListItem> {
    ...
    const hasUpdates = data.code !== undefined || data.name !== undefined ||
      data.displayOrder !== undefined || data.description !== undefined;
