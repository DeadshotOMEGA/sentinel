...
class TagService {
  ...
  async transferLockupTag(
    ...
    // Get the Lockup tag
    const lockupTag = await prisma.tag.findUnique({
      where: { name: 'Lockup' },
    });
    ...
    if (!lockupTag) {
      throw new NotFoundError(
        'TAG_NOT_FOUND',
        'Lockup tag not found in database',
        'The Lockup tag must be created before transfers can occur.'
      );
    }
    ...
    if (!targetMember) {
      throw new NotFoundError(
        ...
        'The member you are trying to transfer the Lockup tag to does not exist.'
      );
    }
    ...
    await prisma.$transaction([
      ...
      prisma.responsibilityAuditLog.create({
        data: {
          memberId: toMemberId,
          tagName: 'Lockup',
          action: 'transferred',
