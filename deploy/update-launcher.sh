...
prompt_version_terminal() {
  ...
  local version=""
  printf '\nSentinel Update Launcher\n'
  printf 'Enter target release version tag (example: v1.1.10)\n'
...
prompt_version_zenity() {
  local default_version="${1:-v1.0.0}"
  zenity --entry \
    --title="Update Sentinel Appliance" \
    --text="Enter target Sentinel version tag (example: v1.1.10):" \
