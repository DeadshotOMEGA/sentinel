export interface TVConfig {
  displayMode: 'unit-overview' | 'division-split' | 'combined' | 'event-only'
...
async function fetchConfigFile(): Promise<TVConfig> {
...
function validateConfig(data: unknown): TVConfig {
  ...
  return {
    displayMode: config.displayMode as 'unit-overview' | 'division-split' | 'combined' | 'event-only',
    refreshInterval: config.refreshInterval,
    activityFeedEnabled: config.activityFeedEnabled,
    eventId: config.eventId as string | null,
    eventName: config.eventName as string | null,
